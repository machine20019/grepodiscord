...
class Bot {
   ...
   */
  loadCommands() {
    fs.readdirSync(this.config.commandPath).forEach(file => {
      let name = file.replace('.js', ''),
          filepath = this.config.commandPath + "/" + file;
  ...
  serverCreated(server) {
    // logger.debug("Got server created event.");
    logger.debug("Server added: %s (%s)", server.name, server.id);
    if (!this.config.botServerId || server.id !== this.config.botServerId) {
      return;
    }

    this.config.botServer = bot.servers.get("id", this.config.botServerId);
    logger.info("Bot Server: %s (%s)", this.config.botServer.name, this.config.botServer.id);
    
    if (this.config.logChannelId) {
      this.config.logChannel = server.channels.get("id", this.config.logChannelId);
      logger.info("Bot Log Channel: %s (%s)\n", this.config.logChannel.name, this.config.logChannel.id);
    }
  }
  ...
  log (level, message) {
    ...
    this.config.logChannel.sendMessage(message);
  }
  ...
  onMessage (msg) {
    ...
    if (msg.author.id != bot.user.id && (commandIsMention || ['!','.'].indexOf(msg.content[0]) !== -1)) {
