    ...
    if (msg.author.id != bot.user.id && (commandIsMention || this.prefixes.indexOf(msg.content[0]) !== -1)) {
      ...
      if (cmd === 'help') {
        ...
        let msgArray = [],
            ...
            base = chain.reject(o => { return o.permissions || o.hideFromHelp || o.disabled; }).pluck('name').uniq().value(),
            mgr = chain.where({ permissions: "manageServer" }).pluck('name').uniq().value(),
            admin = chain.where({ permissions: "admin" }).pluck('name').uniq().value();
